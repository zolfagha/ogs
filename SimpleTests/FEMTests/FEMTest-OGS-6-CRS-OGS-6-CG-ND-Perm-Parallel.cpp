				...
				double** eqsX, std::map<INDEX_TYPE, INDEX_TYPE> &map_solved_orgEqs)
{
	...
	CPUTimeTimer cpu_trans;
	run_trans.start();
	cpu_trans.start();
	MathLib::CRSMatrix<double, INDEX_TYPE>* transpose_mat (eqsA.getTranspose());
	run_trans.stop();
	cpu_trans.stop();
	std::cout << run_trans.elapsed() << " s [wtime: " << cpu_trans.elapsed() << "]" << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    std::vector<MeshLib::IMesh*> vec_mesh;
    MeshLib::MeshIOOGS::readMesh(strMeshFile, vec_mesh);
    if (vec_mesh.size()==0) {
        std::cout << "Failed to read a mesh file: " << strMeshFile << std::endl;
        return 0;
    }
...
//	}
    ...
    std::cout << "-> Nested dissection [ND]" << std::endl;
    // create time measurement objects
	RunTimeTimer run_timer_nd;
	CPUTimeTimer cpu_timer_nd;
    // calculate the nested dissection reordering
	if (verbose) {
		std::cout << "\t[ND] calculating nested dissection reordering of matrix ... " << std::flush;
	}
	run_timer_nd.start();
	cpu_timer_nd.start();
	...
	CPUTimeTimer cpu_timer_apl_nd;
	if (verbose) {
		std::cout << "\t[ND] applying nested dissection reordering to FEM matrix ... " << std::flush;
	}
	run_timer_apl_nd.start();
	cpu_timer_apl_nd.start();
