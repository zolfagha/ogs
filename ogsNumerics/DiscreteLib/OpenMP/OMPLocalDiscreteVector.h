 ...
 */
template<typename T>
class OMPLocalDiscreteVector : public DiscreteVector<T>
{     
 public:
    ...
    OMPLocalDiscreteVector(size_t local_id, size_t n_global, size_t i_begin, size_t i_end, std::vector<size_t> &ghost_id)
    : DiscreteVector<T>(i_end - i_begin + ghost_id.size()), _ghost_id(ghost_id)
    {
        _local_id = local_id;
        _global_n = n_global;
        _i_start = i_begin;
    ...
    /// destructor
    virtual ~OMPLocalDiscreteVector() {};
