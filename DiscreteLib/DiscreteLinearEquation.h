...
class OMPMasterDiscreteLinearEquation : public IDiscreteLinearEquation
{
public:
    OMPMasterDiscreteLinearEquation()
    {

    }

    virtual ~OMPMasterDiscreteLinearEquation()
    {
        Base::releaseObjectsInStdVector(_local_eqs);
    }

    template<class T_LINEAR_SOLVER, class T_SPARSITY_BUILDER>
    AbstractMeshBasedDiscreteLinearEquation* createLocal(MeshLib::IMesh &msh, T_LINEAR_SOLVER &linear_solver, DofMapManager &dofManager)
    {
        TemplateMeshBasedDiscreteLinearEquation<T_LINEAR_SOLVER, T_SPARSITY_BUILDER> *local = new TemplateMeshBasedDiscreteLinearEquation<T_LINEAR_SOLVER, T_SPARSITY_BUILDER>(msh, linear_solver, dofManager);
        _local_eqs.push_back(local);
        return local;
    }
...
private:
    std::vector<AbstractMeshBasedDiscreteLinearEquation*> _local_eqs;
};
