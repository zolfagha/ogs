...
{
public:
    DDCSubDomain(MeshLib::IMesh &msh, IDDCGlobaLocalMapping &mapping) : _local_msh(&msh), _map_global_local(&mapping)
    {
        _dom_id = 0;
    }
...
public:
    ...
    size_t getTotalNumberOfDecomposedObjects() const {return 0;}; 
...
{     
 public:
    ...
    DDCLocalDiscreteVector(size_t local_id, size_t n_global, size_t i_begin, size_t i_end) : DiscreteVector(i_end - i_begin) 
    {
        _local_id = local_id;
        _global_n = n_global;
        _i_start = i_begin;
    ...
    DDCLocalDiscreteVector(size_t local_id, size_t n_global, size_t i_begin, size_t i_end, std::vector<size_t> &ghost_id) : DiscreteVector(i_end - i_begin + ghost_id.size()), _ghost_id(ghost_id)
    {
        _local_id = local_id;
        _global_n = n_global;
        _i_start = i_begin;
...
public:
    ...
    DDCDiscreteVector(size_t n_global, size_t n_divide)
    {
        _global_n = n_global;
