...
#ifndef CHEM_ACTIVITY_MODEL_AQ_DVS_H
#define CHEM_ACTIVITY_MODEL_AQ_DVS_H
 ...
 */
class chemActivityModelAqDVS : chemActivityModelAbstract
{
public:
    // destructor
    chemActivityModelAqDVS(MathLib::LocalVector & log_molarity
                           BaseLib::OrderedMap<std::string, ogsChem::ChemComp*> & map_chemComp) 
        : _activity_model( ogsChem::ACT_MOD_AQ_DVS )
    {
        int counter(0); 
                       ...
                       const double Tc = 25.0)
    {
        ...
        for (int i=0; i < _n; i++ )
        {
            zi = _vec_Zi(i); 
            log_f  = -1.0 * _A * zi * zi;
            log_f *= _sqrt_I ./ ( 1.0 + _sqrt_I ) - 0.3 * I; 
            log_f *= _sqrt_I; 
